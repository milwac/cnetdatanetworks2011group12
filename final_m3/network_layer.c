...
void network_send(){
	...
	printf("Min mtu - FRAME_HEADER_SIZE is  : %d\n", frame_len);	
	int int_len = len - sizeof(CnetAddr);	
	//queue packets up
	for(int seqno = 0; int_len > 0; seqno++){
		FRAME f;
		...
		f.payload.flag_offset = (int_len < frame_len) ? true : false;
		getcurrtime(&f.payload.timestamp);
		f.payload.A = seqno;
		//f.payload.B = seqno * frame_len;
		memcpy(&f.payload.data[0], &next->data[seqno * frame_len], f.payload.len);
	//printf("Length of frame to send is : %d and the payload is %s\n", f.payload.len, f.payload.data[0]);	
		f.checksum = 0;
...
void process_frames(int link){
	...
	if(seq_no == node_buffer[source_nodenumber].next_seq_number_to_add){
		...
		if(f->payload.flag_offset == true) {
			//printf("Bytes in the reconstructed message are %d message is %s\n", node_buffer[source_nodenumber].bytes_added, node_buffer[source_nodenumber].incomplete_data[0]);
