...
void handle_ack(int link, FRAME f){
	...
	if(f.payload.dest != nodeinfo.address){
		...
		f.checksum = 0;
		f.checksum = CNET_crc32((unsigned char *)&f, (int)FRAME_HEADER_SIZE);
...
void process_frames(){
	...
	if(seq_no == node_buffer[source_nodenumber].next_seq_number_to_add){
		...
		while(true){
			...
			size_t plen;
			fr = hashtable_find(node_buffer[source_nodenumber].ooo_packets, next_seq_str, &plen);
			if(plen == 0){
				break;
			}
			printf("In While loop : Next frame %d found in HT\n",next_seq);
			//f = hashtable_remove(node_buffer[source_nodenumber].ooo_packets, next_seq_str, &plen);
		...
		if(seq_no > node_buffer[source_nodenumber].next_seq_number_to_add){
			...
			if(plen == 0){
				hashtable_add(node_buffer[source_nodenumber].ooo_packets, seq_str, f, len);
			}
		}
	}
	node_buffer[source_nodenumber].busy = false;
